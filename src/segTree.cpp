...
SegmentTree::SegmentTree(Matrix* matrices, int numMatrices) {
  size = numMatrices;
...
SegmentTree::SegmentTree(int numMatrices) {
  size = numMatrices;
  tree = new Node[4 * size];  // Usando 4 * size para garantir espaço suficiente
...
void SegmentTree::build(int node, int start, int end, const Matrix* matrices) {
  if (start == end) {
...
void SegmentTree::update(int instant, const Matrix& newMatrix) {
  recursiveUpdate(1, 0, size - 1, instant, newMatrix);
}

// Método auxiliar para atualizar a árvore recursivamente
void SegmentTree::recursiveUpdate(int node, int start, int end, int instant, const Matrix& newMatrix) {
  if (start == end) {
...
void SegmentTree::recursivePrint(int node, int start, int end) const {
  if (start == end) {
...
Matrix* SegmentTree::segProduct(int l, int r) const {
  Matrix* segProduct = recursiveProduct(1, 0, size - 1, l, r);
  return segProduct;
}

// Método auxiliar para obter o produto do intervalo recursivamente
Matrix* SegmentTree::recursiveProduct(int node, int start, int end, int l, int r) const {
